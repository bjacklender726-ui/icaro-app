...
export default function Dashboard() {
  const { agendaTasks, studySessions, jobOffers, gymSessions, projectLogs, temarios, level, xp, hiddenModules } = useStore();
  const bg = useColorModeValue('white', 'gray.800');
  ...
  const filteredAgendaTasks = useMemo(() => (hiddenModules || []).includes('agenda') ? [] : agendaTasks, [agendaTasks, hiddenModules]);
  ...
  const todayTasks = useMemo(() => {
    ...
    return filteredAgendaTasks.filter((t) => t.date === today);
  }, [filteredAgendaTasks]);
  ...
  const totalStudyHours = useMemo(() => +(filteredStudySessions.reduce((a, s) => a + (s.duration || 0), 0) / 60).toFixed(1), [filteredStudySessions]);
  const totalGymSessions = filteredGymSessions.length;
  const totalJobOffers = filteredJobOffers.length;
  const totalProjectHours = useMemo(() => +(filteredProjectLogs.reduce((a, l) => a + (l.hours || 0), 0)).toFixed(1), [filteredProjectLogs]);
  ...
  return (
    <Box>
      <SimpleGrid columns={{ base: 1, md: 2, lg: 4 }} spacing={5} mb={6}>
        {showStudy && <StatCard icon={FiClock} label="Horas de Estudio" value={`${totalStudyHours}h`} change={12} color="#4299E1" sub={`${filteredStudySessions.length} sesiones`} />}
        {showGym && <StatCard icon={FiActivity} label="Sesiones de Gym" value={totalGymSessions} change={8} color="#E53E3E" sub="esta semana" />}
        {showTrabajo && <StatCard icon={FiBriefcase} label="Ofertas Enviadas" value={totalJobOffers} change={-3} color="#ED8936" sub="total" />}
        {showProyectos && <StatCard icon={FiGrid} label="Horas Proyectos" value={`${totalProjectHours}h`} change={20} color="#9F7AEA" sub="invertidas" />}
      </SimpleGrid>
        ...
        <Box bg={bg} p={5} borderRadius="xl" boxShadow="md" border="1px solid" borderColor={borderColor}>
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={productivityData}>
              ...
              {showStudy && <Bar dataKey="estudio" fill="#4299E1" radius={[4, 4, 0, 0]} />}
              {showGym && <Bar dataKey="gimnasio" fill="#E53E3E" radius={[4, 4, 0, 0]} />}
              {showProyectos && <Bar dataKey="proyectos" fill="#9F7AEA" radius={[4, 4, 0, 0]} />}
            </BarChart>
          </ResponsiveContainer>
        </Box>
      ...
      <Grid templateColumns={{ base: '1fr', md: '1fr 1fr' }} gap={5}>
        ...
        <Box bg={bg} p={5} borderRadius="xl" boxShadow="md" border="1px solid" borderColor={borderColor}>
          ...
          <VStack align="stretch" spacing={2} maxH="200px" overflowY="auto">
            {[...filteredStudySessions, ...filteredGymSessions].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 5).map((s) => (
